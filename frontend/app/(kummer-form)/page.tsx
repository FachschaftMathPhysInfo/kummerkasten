"use client";

import AboutSection from './form-about';
import FAQSection from './form-faq';
import FormUi from './form-ui';
import Image from 'next/image';

export default function KummerkastenPage() {
  return (
    <main className="min-h-screen flex flex-col items-center justify-start py-10 px-4 sm:px-6 lg:px-8 space-y-8 max-w mx-auto">

      <div className="flex items-center">
         <Image
          src="/logo.png"
          alt="Kummerkasten Logo"
          width={1024}
          height={1024}
          className="w-16 h-16 sm:w-16 sm:h-16 lg:w-20 lg:h-20 xl:w-24 xl:h-24 rounded-full mr-4" 
        />
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold tracking-tight text-foreground">Kummerkasten</h1>
      </div>
      
      <AboutSection/>

      <FormUi/>

      <FAQSection/>
    </main>
  );
}
